...
def plot_util(base_dir, b, h):
    ...
    for i in range(0, len(x_axis_list), 8):
        if i == 0:
            labels = ['L1 Utilization', 'L2 Utilization', 'DRAM Utilization']
        else:
            labels = [None, None, None]
        plt.plot(x_axis_list[i:i+8], l1_util_list[i:i+8], label=labels[0], marker='o', color=colors[0])
        plt.plot(x_axis_list[i:i+8], l2_util_list[i:i+8], label=labels[1], marker='s', color=colors[1])
        plt.plot(x_axis_list[i:i+8], dram_util_list[i:i+8], label=labels[2], marker='^', color=colors[2])
    # Only legend once
    plt.xlabel('Shape (m x n x d)')
    plt.ylabel('Utilization (%)')
    plt.title(f'Utilization vs Shape for {b}x{h}')
    plt.xticks(rotation=90, fontsize=6)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{base_dir}/util_plot_{b}x{h}.png")
